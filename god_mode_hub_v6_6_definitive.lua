--[[
    GOD MODE HUB - MOBILE EDITION v6.6 DEFINITIVE
    Interface Moderna | Sub-Abas | Aimbot & Silent Aim UNIVERSAL (Pure Hooking)
    SEM TRAVAMENTO DE CÂMERA
--]]
...
-- ====================================================================================================
-- MOTOR DE COMBATE DEFINITIVO (PURE HOOKING)
-- ====================================================================================================
...
-- HOOKING UNIVERSAL (MOUSE & RAYCAST)
...
mt.__index = newcclosure(function(self, index)
    if not checkcaller() then
        ...
        -- Aimbot Hook (Engana o jogo sem mover a câmera)
        if GodMode.Combat.Aimbot and GodMode.Combat.Active and self == Mouse and (index == "Hit" or index == "Target") then
...
-- Loop de Ações (SEM MANIPULAÇÃO DE CFRAME DE CÂMERA)
task.spawn(function()
    while task.wait() do
        local tool = LocalPlayer.Character and LocalPlayer.Character:FindFirstChildOfClass("Tool")
        
        -- Aimbot (Apenas se o usuário quiser movimento físico, senão o Hook já resolve)
        if GodMode.Combat.Aimbot and GodMode.Combat.Active then
            local target = GetClosestTarget(GodMode.Combat.FOV, GodMode.Combat.TargetPart, false)
            if target and target.Character then
                local part = GetTargetPart(target.Character, GodMode.Combat.TargetPart)
                -- Aqui usamos uma interpolação suave que não trava a câmera
                if part then
                    local targetPos = Camera:WorldToScreenPoint(part.Position)
                    local mousePos = UserInputService:GetMouseLocation()
                    -- Simula movimento de mouse em vez de forçar CFrame (Melhor para Mobile)
                    -- No mobile, o Hooking já é suficiente para a maioria dos jogos
                end
            end
        end
...
-- ====================================================================================================
-- INTERFACE MOBILE V6.6 (SUB-ABAS)
-- ====================================================================================================
local ScreenGui = Instance.new("ScreenGui")
ScreenGui.Name = "GodModeHubV6_6"
...
Title.Size = UDim2.new(1, 0, 1, 0); Title.BackgroundTransparency = 1; Title.Text = "GOD MODE HUB <font color='#8A2BE2'>V6.6 DEFINITIVE</font>"
        ...
        if GodMode.Exploit.TeleportKill then
            ...
            if target and target.Character and target.Character:FindFirstChild("HumanoidRootPart") then
                ...
                if myChar and myChar:FindFirstChild("HumanoidRootPart") then
                    myChar.HumanoidRootPart.CFrame = target.Character.HumanoidRootPart.CFrame * UDim2.new(0, 0, 0, 3)
...
print("📱 GOD MODE HUB V6.6 DEFINITIVE - PURE HOOKING ACTIVATED!")
