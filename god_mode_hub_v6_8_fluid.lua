--[[
    GOD MODE HUB - MOBILE EDITION v6.8 FLUID
    Interface Moderna | Sub-Abas | Silent Aim & Aimbot SEM CONGELAR TELA
    VERSÃO MASSIVA E CORRIGIDA
--]]
...
local GodMode = {
    ...
    Visuals = {
        ...
        TracerPos = "Bottom", Names = false, HealthBar = false, Distance = false,
        Chams = false, ChamsColor = Color3.fromRGB(138, 43, 226), ChamsOutline = Color3.fromRGB(255, 255, 255),
        TeamCheck = false, Color = Color3.fromRGB(0, 255, 150), MaxDistance = 2000
    },
    Movement = {
        Bhop = false, InfJump = false, Noclip = false,
        WalkSpeed = 16, JumpPower = 50, SpeedEnabled = false, JumpEnabled = false
    },
    Exploit = { TeleportKill = false, KillDelay = 0.5, Invisibility = false, Fly = false, FlySpeed = 50 },
    GunMod = { FastFire = false, InstantReload = false, NoRecoil = false, NoSpread = false, InfiniteAmmo = false, RangeHack = false },
...
FOVCircle.Thickness = 2; FOVCircle.NumSides = 60; FOVCircle.Visible = false; FOVCircle.Color = GodMode.Config.AccentColor

local SilentFOVCircle = Drawing.new("Circle")
SilentFOVCircle.Thickness = 2; SilentFOVCircle.NumSides = 60; SilentFOVCircle.Visible = false; SilentFOVCircle.Color = Color3.fromRGB(255, 50, 50)
...
local function RemoveESP(player)
    if ESP_Data[player] then
        ...
        pcall(function()
            data.Box:Remove(); data.Name:Remove(); data.Health:Remove(); data.HealthBg:Remove(); data.Line:Remove(); data.Distance:Remove()
...
local function CreateESP(player)
    ...
    local data = {
        ...
        Health = Drawing.new("Line"), HealthBg = Drawing.new("Line"), Line = Drawing.new("Line"),
        Corners = {}, Skeleton = {}, Highlight = nil
    }
    data.Box.Thickness = 2; data.Name.Size = 16; data.Name.Center = true; data.Name.Outline = true; data.Name.Font = 2
    data.Distance.Size = 14; data.Distance.Center = true; data.Distance.Outline = true; data.Distance.Font = 2
    data.HealthBg.Thickness = 4; data.HealthBg.Color = Color3.new(0, 0, 0); data.Health.Thickness = 2; data.Line.Thickness = 1.5
    for i = 1, 8 do local l = Drawing.new("Line") l.Thickness = 2; l.Visible = false; table.insert(data.Corners, l) end
    for i = 1, 15 do local l = Drawing.new("Line") l.Thickness = 1.5; l.Visible = false; table.insert(data.Skeleton, l) end
    ESP_Data[player] = data
end

-- ====================================================================================================
-- MOTOR DE COMBATE (FLUIDO - SEM CONGELAR TELA)
-- ====================================================================================================
        ...
        if p ~= LocalPlayer and p.Character then
            ...
            if part then
                ...
                if ignoreFOV then
                    ...
                    if mag < dist and IsVisible(part) then target = p; dist = mag end
                elseif onScreen then
                    local mag = (Vector2.new(pos.X, pos.Y) - center).Magnitude
                    if mag <= fov and mag < dist and IsVisible(part) then target = p; dist = mag end
                end
            end
        end
    end
...
-- LÓGICA DE REDIRECIONAMENTO (SEM HOOKING DE CÂMERA)
...
-- Interceptação de Disparo (Apenas para redirecionar a bala, sem travar a tela)
    ...
    if GodMode.Combat.Aimbot and GodMode.Combat.Active then
        ...
        if target and target.Character then
            ...
            if part then
                ...
                -- Em vez de forçar CFrame, apenas sugere uma direção suave
...
-- ====================================================================================================
-- INTERFACE MOBILE V6.8 (MASSIVE UI)
-- ====================================================================================================
local ScreenGui = Instance.new("ScreenGui")
ScreenGui.Name = "GodModeHubV6_8"
...
Title.Size = UDim2.new(1, 0, 1, 0); Title.BackgroundTransparency = 1; Title.Text = "GOD MODE HUB <font color='#8A2BE2'>V6.8 FLUID</font>"
...
AddToggle(VisualsPage, "Skeleton ESP", false, function(v) GodMode.Visuals.Skeletons = v end)
AddToggle(VisualsPage, "Names ESP", false, function(v) GodMode.Visuals.Names = v end)
AddToggle(VisualsPage, "Health Bar", false, function(v) GodMode.Visuals.HealthBar = v end)
AddToggle(VisualsPage, "Distance ESP", false, function(v) GodMode.Visuals.Distance = v end)
AddToggle(VisualsPage, "Chams (Wallhack)", false, function(v) GodMode.Visuals.Chams = v end)
AddToggle(VisualsPage, "Tracers (Lines)", false, function(v) GodMode.Visuals.Lines = v end)
AddSelector(VisualsPage, "Tracer Position", {"Bottom", "Top", "Middle"}, "Bottom", function(v) GodMode.Visuals.TracerPos = v end)
AddToggle(VisualsPage, "Team Check", false, function(v) GodMode.Visuals.TeamCheck = v end)
AddSlider(VisualsPage, "Max ESP Distance", 100, 5000, 2000, function(v) GodMode.Visuals.MaxDistance = v end)

-- ABA MOVEMENT
local MovePage = CreateScroll(CreateTab("Move", "🏃"))
AddToggle(MovePage, "Enable Speed Hack", false, function(v) GodMode.Movement.SpeedEnabled = v end)
AddSlider(MovePage, "WalkSpeed", 16, 500, 16, function(v) GodMode.Movement.WalkSpeed = v end)
AddToggle(MovePage, "Enable Jump Hack", false, function(v) GodMode.Movement.JumpEnabled = v end)
AddSlider(MovePage, "JumpPower", 50, 500, 50, function(v) GodMode.Movement.JumpPower = v end)
...
AddToggle(GunPage, "No Spread", false, function(v) GodMode.GunMod.NoSpread = v end)
AddToggle(GunPage, "Infinite Ammo", false, function(v) GodMode.GunMod.InfiniteAmmo = v end)
AddToggle(GunPage, "Range Hack", false, function(v) GodMode.GunMod.RangeHack = v end)
...
AddSlider(ExploitPage, "TP Kill Delay", 0.1, 5, 0.5, function(v) GodMode.Exploit.KillDelay = v end)
AddToggle(ExploitPage, "Invisibility", false, function(v) GodMode.Exploit.Invisibility = v end)
AddToggle(ExploitPage, "Fly Mode", false, function(v) GodMode.Exploit.Fly = v end)
AddSlider(ExploitPage, "Fly Speed", 10, 500, 50, function(v) GodMode.Exploit.FlySpeed = v end)

-- ====================================================================================================
-- SISTEMAS DE RENDERIZAÇÃO E LOOPS
-- ====================================================================================================
            ...
            if dist <= GodMode.Visuals.MaxDistance then
                ...
                if onScreen then
                    ...
                    if GodMode.Visuals.Chams then
                        ...
                        data.Highlight.FillColor = GodMode.Visuals.ChamsColor; data.Highlight.OutlineColor = GodMode.Visuals.ChamsOutline; data.Highlight.FillTransparency = 0.5; data.Highlight.Enabled = true
                    ...
                    data.Name.Visible = GodMode.Visuals.Names; data.Name.Text = player.Name; data.Name.Position = Vector2.new(pos.X, y - 20); data.Name.Color = Color3.new(1, 1, 1)
                    data.Distance.Visible = GodMode.Visuals.Distance; data.Distance.Text = "[" .. math.floor(dist) .. "m]"; data.Distance.Position = Vector2.new(pos.X, y + h + 5); data.Distance.Color = Color3.new(1, 1, 1)
                    
                    if GodMode.Visuals.HealthBar then
                        local healthPercent = hum.Health / hum.MaxHealth; local barHeight = h * healthPercent
                        data.HealthBg.Visible = true; data.HealthBg.From = Vector2.new(x - 6, y + h); data.HealthBg.To = Vector2.new(x - 6, y)
                        data.Health.Visible = true; data.Health.From = Vector2.new(x - 6, y + h); data.Health.To = Vector2.new(x - 6, y + h - barHeight); data.Health.Color = Color3.fromHSV(healthPercent * 0.3, 1, 1)
                    else data.Health.Visible = false; data.HealthBg.Visible = false end
                    
                    data.Line.Visible = GodMode.Visuals.Lines
                    local startPos = Vector2.new(Camera.ViewportSize.X / 2, Camera.ViewportSize.Y)
                    if GodMode.Visuals.TracerPos == "Top" then startPos = Vector2.new(Camera.ViewportSize.X / 2, 0)
                    elseif GodMode.Visuals.TracerPos == "Middle" then startPos = center end
                    ...
                    if GodMode.Visuals.Skeletons then
                        ...
                        for i, bone in ipairs(bones) do
                            ...
                            elseif i <= #data.Skeleton then data.Skeleton[i].Visible = false end
                        end
                    ...
                    data.Box.Visible = false; data.Name.Visible = false; data.Distance.Visible = false; data.Health.Visible = false; data.HealthBg.Visible = false; data.Line.Visible = false
                end
            else
                for i=1,8 do data.Corners[i].Visible = false end; for _, line in pairs(data.Skeleton) do line.Visible = false end
                data.Box.Visible = false; data.Name.Visible = false; data.Distance.Visible = false; data.Health.Visible = false; data.HealthBg.Visible = false; data.Line.Visible = false
        ...
        if tool then
            for _, v in pairs(tool:GetDescendants()) do
                if v:IsA("NumberValue") or v:IsA("IntValue") then
                    ...
                    if GodMode.GunMod.FastFire and (n:find("fire") or n:find("delay")) then v.Value = 0.01 end
                    if GodMode.GunMod.InstantReload and (n:find("reload")) then v.Value = 0.01 end
                    if GodMode.GunMod.NoRecoil and (n:find("recoil") or n:find("kick")) then v.Value = 0 end
                    if GodMode.GunMod.NoSpread and (n:find("spread")) then v.Value = 0 end
                    if GodMode.GunMod.InfiniteAmmo and (n:find("ammo") or n:find("clip")) then v.Value = 999 end
                end
            end
        end
    end
...
RunService.PreSimulation:Connect(function()
    ...
    if hum then
        ...
        if GodMode.Movement.Bhop and hum.MoveDirection.Magnitude > 0 and hum.FloorMaterial ~= Enum.Material.Air then hum:ChangeState(Enum.HumanoidStateType.Jumping) end
    end
...
FloatingButton.MouseButton1Click:Connect(function()
    ...
    else MainFrame.Visible = true; local targetSize = UserInputService.TouchEnabled and UDim2.new(0, 480, 0, 320) or UDim2.new(0, 550, 0, 380); MainFrame:TweenSize(targetSize, "Out", "Back", 0.3, true) end
end)
...
print("📱 GOD MODE HUB V6.8 FLUID - CARREGADO!")
