--[[
    GOD MODE HUB - MOBILE EDITION v6.2
    Interface Moderna | Totalmente Responsiva | Silent Aim Added
...
local GodMode = {
    Combat = {
        ...
        SilentFOV = 200,
        ShowSilentFOV = true,
        SilentTargetPart = "Head", -- "Head", "Torso", "Legs"
...
local function GetTargetPart(character, partName)
    ...
    elseif partName == "Legs" then return character:FindFirstChild("LeftLowerLeg") or character:FindFirstChild("LeftLeg")
    end
...
-- Hook para Silent Aim (Redirecionamento de Muzzle)
RunService.Heartbeat:Connect(function()
    ...
    if GodMode.Combat.SilentAim then
        ...
        if target and target.Character then
            ...
            if part then
                muzzle.CFrame = CFrame.new(muzzle.Position, part.Position)
            end
        end
    ...
    while task.wait() do
        ...
        if GodMode.Combat.Aimbot and GodMode.Combat.Active then
            ...
            if target and target.Character then
                local part = GetTargetPart(target.Character, GodMode.Combat.TargetPart)
                if part then
                    Camera.CFrame = Camera.CFrame:Lerp(CFrame.new(Camera.CFrame.Position, part.Position), GodMode.Combat.Smoothness)
                end
            end
        end
...
-- ====================================================================================================
-- INTERFACE MOBILE V6.2
-- ====================================================================================================
local ScreenGui = Instance.new("ScreenGui")
ScreenGui.Name = "GodModeHubV6_2"
...
FloatingButton.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
        draggingFloat = true; dragStart = input.Position; startPos = FloatingButton.Position
    end
end)
...
Title.Size = UDim2.new(1, 0, 1, 0); Title.BackgroundTransparency = 1; Title.Text = "GOD MODE HUB <font color='#8A2BE2'>V6.2</font>"
...
local function CreateTab(name, icon)
    ...
    page.Size = UDim2.new(1, 0, 1, 0); page.BackgroundTransparency = 1; page.Visible = false; page.ScrollBarThickness = 2; page.CanvasSize = UDim2.new(0, 0, 0, 0); page.Parent = Content
...
local function AddSection(tab, text)
    ...
    label.Size = UDim2.new(0.95, 0, 0, 30); label.BackgroundTransparency = 1; label.Text = "--- " .. text .. " ---"
    label.TextColor3 = GodMode.Config.AccentColor; label.Font = Enum.Font.GothamBold; label.TextSize = 12; label.Parent = TabPages[tab].Page
end

local function AddToggle(tab, text, default, callback)
    local frame = Instance.new("Frame")
    frame.Size = UDim2.new(0.95, 0, 0, 45); frame.BackgroundColor3 = Color3.fromRGB(30, 30, 40); frame.Parent = TabPages[tab].Page
...
local function AddSlider(tab, text, min, max, default, callback)
    local frame = Instance.new("Frame")
    frame.Size = UDim2.new(0.95, 0, 0, 65); frame.BackgroundColor3 = Color3.fromRGB(30, 30, 40); frame.Parent = TabPages[tab].Page
    Instance.new("UICorner", frame).CornerRadius = UDim.new(0, 10)
    local label = Instance.new("TextLabel")
...
local function AddButton(tab, text, callback)
    local btn = Instance.new("TextButton")
    btn.Size = UDim2.new(0.95, 0, 0, 45); btn.BackgroundColor3 = GodMode.Config.AccentColor; btn.Text = text; btn.TextColor3 = Color3.fromRGB(255, 255, 255); btn.Font = Enum.Font.GothamBold; btn.TextSize = 14; btn.Parent = TabPages[tab].Page
...
AddSlider("Combat", "Aimbot FOV", 50, 800, 150, function(v) GodMode.Combat.FOV = v end)
AddToggle("Combat", "Show Aimbot FOV", true, function(v) GodMode.Combat.ShowFOV = v end)
AddSelector("Combat", "Aimbot Target", {"Head", "Torso", "Legs"}, "Head", function(v) GodMode.Combat.TargetPart = v end)
AddButton("Combat", "🎯 ACTIVATE AIMBOT (HOLD)", function() GodMode.Combat.Active = true task.wait(0.1) end)

AddSection("Combat", "SILENT AIM")
AddToggle("Combat", "Enable Silent Aim", false, function(v) GodMode.Combat.SilentAim = v end)
AddSlider("Combat", "Silent FOV", 50, 800, 200, function(v) GodMode.Combat.SilentFOV = v end)
AddToggle("Combat", "Show Silent FOV", true, function(v) GodMode.Combat.ShowSilentFOV = v end)
AddSelector("Combat", "Silent Target", {"Head", "Torso", "Legs"}, "Head", function(v) GodMode.Combat.SilentTargetPart = v end)

AddSection("Combat", "OTHERS")
...
AddSelector("Visuals", "Tracer Position", {"Bottom", "Top"}, "Bottom", function(v) GodMode.Visuals.TracerPos = v end)
...
print("📱 GOD MODE HUB V6.2 - SILENT AIM ADDED!")
